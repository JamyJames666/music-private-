...
export default function NowPlaying({ status, token, guildId, onRefresh, onPositionChange }: Props) {
  ...
  return (
    // Fills the entire container — art edge-to-edge, overlay inside at bottom
    <div className="relative w-full h-full overflow-hidden">

      {/* Album art — fills the full rectangular area */}
      {np?.thumbnailUrl ? (
        <img
          src={np.thumbnailUrl}
          alt={np?.title ?? ''}
        ...
        <div className="absolute inset-0"
          style={{ background: 'linear-gradient(135deg,#1a1030,#0a0820)' }}>
          <div className="absolute inset-0 flex items-center justify-center">
            <Music size={72} style={{ color: active ? '#7c3aed' : '#333' }} />
          </div>
        </div>
      )}

      {/* Vignette — bottom-heavy so controls stay readable */}
      ...
      {/* Frosted glass overlay — same compact size as before, full width, more transparent */}
      {active && (
        <div
          className="absolute left-3 right-3 bottom-3 z-10 rounded-2xl px-4 py-3 space-y-2.5"
          style={{
